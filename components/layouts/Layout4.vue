    ...
    <ClientOnly>
      <div class="relative max-h-screen overflow-hidden z-1">
        ...
        <NuxtImg
          v-if="data.thumbnail"
          :src="data.thumbnail"
          class="w-screen opacity-80 bg-cover animate-camera-view"
        ...
        <div
          class="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-0 animate-fade animate-once animate-delay-[500ms]"
          v-if="imageLoaded"
        >
          <div class="container p-4">
            <div>
              <h1 class="text-black text-4xl md:text-6xl lg:text-8xl font-bold">{{ data.title }}</h1>
...
.animate-camera-view {
  animation: camera-view 30s infinite;
}
</style>
